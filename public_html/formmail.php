...
$recipient = getenv('FORMMAIL_RECIPIENT') ?: "tpiacentino@gmail.com";

// bcc emails (separate multiples with commas (,))
$bcc = getenv('FORMMAIL_BCC') ?: "karchuf@gmail.com";

// referers.. domains/ips that you will allow forms to
// reside on.
$referers_env = getenv('FORMMAIL_REFERERS');
$referers = $referers_env ? explode(',', $referers_env) : array('teresapiacentino.com', 'www.teresapiacentino.com', 'localhost');
...
function check_referer($referers)
{
   if (count($referers)) {
      ...
      for ($x = 0; $x < count($referers); $x++) {
         if (preg_match('/' . preg_quote($referers[$x], '/') . '/i', $referer)) {
            $found = true;
         }
      }
      if ($referer == "")
...
for ($i = 0; $i < count($recipient_in); $i++) {
   ...
   if (!preg_match("/^[_\\.0-9a-z-]+@([0-9a-z][0-9a-z-]+\\.)+[a-z]{2,6}$/i", $recipient_to_test)) {
...
if (($ZIP_CODE) || ($zip_code)) {
   ...
   if ($ZIP_CODE)
      $zip_code = trim($ZIP_CODE);
   if (!preg_match("/(^[0-9]{5})-([0-9]{4}$)/", trim($zip_code)) && (!preg_match("/^[a-zA-Z][0-9][a-zA-Z]\s[0-9][a-zA-Z][0-9]$/", trim($zip_code))) && (!preg_match("/(^[0-9]{5})/", trim($zip_code))))
...
} elseif ((preg_match('/^order:.*,.*/', $sort)) && ($list = explode(',', preg_replace('/^order:/', '', $sort)))) {
   $sort = $list;
}
...
$subject = "Consulta Cuadros - " . $_POST['name'];

mail_it(stripslashes($content), ($subject) ? stripslashes($subject) : "Consulta Cuadros", $email, $recipient);
